...
public class TransactionController {
    ...
    private void addTransaction() {
        try {
            ...
            if (!selectedBook.isIs_available()) {
                showAlert("Book Unavailable", "This book is not available for checkout.");
                return;
            }
        ...
        if (selectedTransaction != null) {
            ...
            alert.showAndWait().ifPresent(buttonType -> {
                if (buttonType == buttonTypeDelete) {
                    ...
                    // Make the book available again after the transaction is deleted
                    Book returnedBook = selectedTransaction.getBookId();
                    returnedBook.setIs_available(true);
