...
public class TransactionDaoImpl implements TransactionDao {
    ...
    public void addTransaction(Transaction transaction) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (!isBookAvailable(transaction.getBookId().getBook_id())) {
                System.out.println("Book is not available. Cannot check out.");
                return;
            }
            ...
            if (rowsAffected > 0) {
                updateBookAvailability(transaction.getBookId().getBook_id(), false); // Mark book as unavailable
                System.out.println("Book checked out successfully.");
            } else {
                System.out.println("Failed to check out the book after transaction.");
            }
    ...
    public void deleteTransaction(int transactionId) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                if (!returned) {
                    updateBookAvailability(bookId, true); // Mark book as available only if it was not returned
                }
                ...
                if (rowsAffected > 0) {
                    System.out.println("Transaction deleted successfully and book availability updated.");
                } else {
                    System.out.println("Transaction not deleted.");
                }
            } else {
            ...
            if (rs.next()) {
                ...
                if (!returned) {
                    ...
                    if (rowsAffected > 0) {
                        ...
                        System.out.println("Transaction marked as returned and book availability updated.");
                    } else {
                        System.out.println("Failed to update the transaction.");
                    }
                } else {
